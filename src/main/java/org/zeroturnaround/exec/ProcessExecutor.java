 ...
 * {@link #redirectOutput(OutputStream)}, or any of the
 * <code>redirectOutputAs*</code> methods.to override it.</li>
 * <li>Any exit code is allowed. Use {@link #exitValues(Integer...)} to override
 * it.</li>
 * <li>In case of timeout or cancellation {@link Process#destroy()} is invoked.
 * </li></li>
 * </p>
 *
...
public class ProcessExecutor {
	...
	/**
	 * Environment variables which are added (removed in case of
	 * <code>null</code> values) to the process being started.
	 */
	...
	/**
	 * Creates new {@link ProcessExecutor} instance for the given program and
	 * its arguments.
	 * 
	 * @param command
	 ...
	 * working directory. The returned value may be {@code null} -- this means
	 * to use the working directory of the current Java process, usually the
	 * directory named by the system property {@code user.dir}, as the working
	...
	/**
	 * Sets this working directory for the process being executed. The argument
	 ...
	 * property <code>user.dir</code>, as the working directory of the child
	 * process.
	 * </p>
	 *
	 * @param directory
	...
	/**
	 * Returns this process executor's additional environment variables. The
	 ...
	 *
	 * @param env
	 *            environment variables added to the process being executed.
	 * @return This process executor.
	 */
	...
	/**
	 * Adds a single additional environment variable for the process being
	 * executed.
	 *
	 * @param name
	 *            name of the environment variable added to the process being
	 *            executed.
	 * @param value
	 *            value of the environment variable added to the process being
	 *            executed.
	 * @return This process executor.
	 *
	 * @since 1.7
	 ...
	 * subprocesses will be merged with the standard output. This makes it
	 * easier to correlate error messages with the corresponding output. The
	...
	/**
	 * Allows only <code>0</code> as the exit value for the process being
	 * executed.
	 *
	 * @return This process executor.
	 */
	 ...
	 *
	 * @param exitValue
	 *            single exit value or <code>null</code> if all exit values are
	 *            allowed.
	 * @return This process executor.
	 */
	 ...
	 *
	 * @param exitValues
	 *            set of exit values or <code>null</code> if all exit values are
	 *            allowed.
	 * @return This process executor.
	 */
	...
	/**
	 * Sets a timeout for the process being executed. When this timeout is
	 * reached a {@link TimeoutException} is thrown and the process is
	 * destroyed. This only applies to <code>execute</code> methods not
	...
	/**
	 * Sets the helper for stopping the process in case of timeout or
	 * cancellation.
	 * <p>
	 ...
	 *
	 * @param stopper
	 *            helper for stopping the process (<code>null</code> means
	...
	/**
	 * Sets a stream handler for the process being executed. This will overwrite
	 * any stream redirection that was previously set to use the provided
	 * handler.
	 * 
	 * @return This process executor.
	 */
	...
	/**
	 * Sets a timeout for closing standard streams of the process being
	 * executed. When this timeout is reached we log a warning but consider that
	 * the process has finished. We also flush the streams so that all output
	 * read so far is available.
	 * <p>
	 * This can be used on Windows in case a process exits quickly but closing
	 * the streams blocks forever.
	 * </p>
	 * <p>
	 * Closing timeout must fit into the general execution timeout (see
	 * {@link #timeout(long, TimeUnit)}). By default there's no closing timeout.
	 *
	 * @param timeout
	...
	/**
	 * Sets the input stream to redirect to the process' input stream. If this
	 * method is invoked multiple times each call overwrites the previous.
	 *
	 * @param input
	 *            input stream that will be written to the process input stream
	 *            (<code>null</code> means nothing will be written to the
	 *            process input stream).
	 * @return This process executor.
	 */
	public ProcessExecutor redirectInput(InputStream input) {
		PumpStreamHandler pumps = pumps();
		// Only set the input stream handler, preserve the same output and error
		// stream handler
		return streams(new PumpStreamHandler(pumps == null ? null : pumps.getOut(),
				pumps == null ? null : pumps.getErr(), input));
	}

	/**
	 * Redirects the process' output stream to given output stream. If this
	 * method is invoked multiple times each call overwrites the previous. Use
	 * {@link #redirectOutputAlsoTo(OutputStream)} if you want to redirect the
	 * output to multiple streams.
	 *
	 * @param output
	 *            output stream where the process output is redirected to
	...
	public ProcessExecutor redirectOutput(OutputStream output) {
		...
		PumpStreamHandler pumps = pumps();
		// Only set the output stream handler, preserve the same error stream
		// handler
	...
	/**
	 * Redirects the process' error stream to given output stream. If this
	 * method is invoked multiple times each call overwrites the previous. Use
	 * {@link #redirectErrorAlsoTo(OutputStream)} if you want to redirect the
	 * error to multiple streams.
	 * <p>
	 * Calling this method automatically disables merging the process error
	 * stream to its output stream.
	 * </p>
	 *
	 * @param output
	 *            output stream where the process error is redirected to
	 *            (<code>null</code> means {@link NullOutputStream} which acts
	 *            like a <code>/dev/null</code>).
	 * @return This process executor.
	 */
	public ProcessExecutor redirectError(OutputStream output) {
		if (output == null)
			output = NullOutputStream.NULL_OUTPUT_STREAM;
		PumpStreamHandler pumps = pumps();
		// Only set the error stream handler, preserve the same output stream
		// handler
	...
	/**
	 * Redirects the process' output stream also to a given output stream. This
	 * method can be used to redirect output to multiple streams.
	 *
	 * @return This process executor.
	 */
	...
	/**
	 * Redirects the process' error stream also to a given output stream. This
	 * method can be used to redirect error to multiple streams.
	 * <p>
	 * Calling this method automatically disables merging the process error
	 * stream to its output stream.
	 * </p>
	 ...
	 *             if the current stream handler is not an instance of
	 *             {@link PumpStreamHandler}.
	 *
	 * @see #streams()
	 */
	 ...
	 *
	 * <p>
	 * If this property is <code>true</code>, the process output should be read
	 * to a buffer and returned by {@link ProcessResult#output()}. The initial
	...
	/**
	 * Adds a process destroyer to be notified when the process starts and
	 * stops.
	 * 
	 * @param destroyer
	 *            helper for destroying all processes on certain event such as
	...
	/**
	 * Sets the process destroyer to be notified when the process starts and
	 * stops.
	 * <p>
	 * This methods always removes any other {@link ProcessDestroyer}
	 * registered. Use {@link #addDestroyer(ProcessDestroyer)} to keep the
	 * existing ones.
	 *
	 * @param destroyer
	 *            helper for destroying all processes on certain event such as
	 ...
	 * executed). If this VM gets killed the started process may not get
	 * destroyed.
	 * <p>
	 ...
	 * 
	 * @param listener
	 *            process event handler to be set (maybe <code>null</code>).
	 *
	 * @return This process executor.
	 */
	 ...
	 * 
	 * @param listener
	 *            process event handler to be added.
	 *
	 * @return This process executor.
	 */
	 ...
	 * 
	 * @param listener
	 *            process event handler to be removed.
	 *
	 * @return This process executor.
	 */
	...
	/**
	 * Unregister existing process event handlers of given type or its
	 * sub-types.
	 * 
	 * @param listenerType
	 ...
	 * timeout occurs or the caller thread gets interrupted. In the latter cases
	 * the process gets destroyed as well.
	 *
	 * @return exit code of the finished process.
	 * @throws IOException
	 *             an error occurred when process was started or stopped.
	 * @throws InterruptedException
	 *             this thread was interrupted.
	 * @throws TimeoutException
	 *             timeout set by {@link #timeout(long, TimeUnit)} was reached.
	 * @throws InvalidExitValueException
	 *             if invalid exit value was returned (@see
	...
	/**
	 * Check the exit value of given process result. This can be used by unit
	 * tests.
	 *
	 * @param result
	 *            process result which maybe constructed by a unit test.
	 * @throws InvalidExitValueException
	 ...
	 * Value passed to {@link #timeout(long, TimeUnit)} is ignored. Use
	 * {@link Future#get()} to wait for the process to finish. Invoke
	 * <code>future.cancel(true);</code> to destroy the process.
	 *
	 ...
	 * @throws IOException
	 *             the process or its stream handlers couldn't start (in the
	...
	private ProcessAttributes getAttributes() {
		return new ProcessAttributes(getCommand(), getDirectory(), new LinkedHashMap<String, String>(environment),
	...
	private WaitForProcess startInternal(Process process, ProcessAttributes attributes, ExecuteStreamHandler streams,
		...
		// Invoke listeners - changing this executor does not affect the started
		// process any more
	 ...
	 * interrupted. In the latter cases the process gets destroyed as well.
	 */
	private ProcessResult waitFor(WaitForProcess task) throws IOException, InterruptedException, TimeoutException {
		...
		if (timeout == null) {
			...
			try {
				...
				// Interrupt the task if it's still running and release the
				// ExecutorService's resources
				service.shutdownNow();
			}
		}
		return result;
	...
	private ExecutorService newExecutor(WaitForProcess task) {
		...
		// If #destroyOnExit() is used we wait for the process to be destroyed
		// anyway
	...
	/**
	 * Fixes the command line arguments on Windows by replacing empty arguments
	 * with <code>""</code>. Otherwise these arguments would be just skipped.
	 *
